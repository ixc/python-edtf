...
def test_trim_struct_time():
    ...
    assert trimmed_st[:6] == (now.year, now.month, now.day, now.hour, now.minute, now.second)
...
def test_roll_negative_time_fields():
    ...
    minute = -74
    second = -253
    assert (-102, 5, 24, 21, 41, 47) == convert._roll_negative_time_fields(year, month, day, hour, minute, second)
