    ...
    def _strict_date(self, lean):
        if lean == EARLIEST:
            try:
                ...
                AttributeError
            ):  # it's a string, or no date. Result depends on the upper date
        ...
        if lean == EARLIEST:
            if self.is_unknown:
                ...
                return dt_to_struct_time(
                    date.min
                )  # from the beginning of time; *ahem, i mean python datetime
        else:
            if self.is_unknown:
...
class PartialUncertainOrApproximate(Date):
    ...
    def __str__(self):
        ...
        y = f"{self.year}{self.year_ua}" if self.year_ua else str(self.year)

        m = f"({self.month}){self.month_ua}" if self.month_ua else str(self.month)

        if self.day:
            d = f"({self.day}){self.day_ua}" if self.day_ua else str(self.day)
        else:
            d = None
