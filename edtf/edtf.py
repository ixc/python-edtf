...
import re
import edtf_date, edtf_interval
...
class EDTF(object):
    ...
    def from_natural_text(cls, text):
        ...
        Rough, ready, partial parser for US natural language date text into an EDTF date.
        http://www.loc.gov/standards/datetime/

        The approach here is to parse the text twice, with different default dates.
        Then compare the results to see what differs - the parts that differ are
        undefined.
        """
        ...
        could_be_century = re.findall(r'(\d{2}00)s', t) #matches on '1800s'. Needs to happen before is_decade.
        t, is_decade = re.subn(r'(\d{3}0)s', r'\1', t) #matches on '1800s' and '1910s'. Removes the 's'. Needs to happen before is_uncertain because e.g. "1860s?"

        is_approximate = re.findall(r'\b(ca?\.?) ?\d{4}', t) #a few abbreviations just before the year
        is_approximate = is_approximate or re.findall(r'\bcirca\b', t) # the word 'circa' anywhere
        is_approximate = is_approximate or re.findall(r'\bapprox', t) # the word 'approx' anywhere
        is_approximate = is_approximate or re.findall(r'\baround\b', t) # the word 'around' anywhere
        is_approximate = is_approximate or re.findall(r'\babout\b', t) # the word 'about' anywhere
        is_approximate = is_approximate or re.findall(r'\b~\d{4}', t) # a ~ before a year-ish number
        is_approximate = is_approximate or re.findall(r'^~', t) # a ~ at the beginning

        t, is_uncertain = re.subn(r'(\d{4})\?', r'\1', t)
        is_uncertain = is_uncertain or re.findall(r'\buncertain', t) # the word uncertain anywhere
        is_uncertain = is_uncertain or re.findall(r'\bmaybe', t) # the word uncertain anywhere
        is_uncertain = is_uncertain or re.findall(r'\bguess', t) # the word uncertain anywhere
